...
private:
    static BOOL TestManualRegistration()
    {
        ...
        BOOL result = RunScriptAndCheckResult(L, L"tests/scripts/state/manual_registration.pil"_embed);
        delete L;
        return result;
    }
    ...
    static BOOL TestGlobalVariables()
    {
        ...
        BOOL result = RunScriptAndCheckResult(L, L"tests/scripts/state/global_variables.pil"_embed);
        delete L;
        return result;
    }
    ...
    static BOOL TestMinimalSetup()
    {
        ...
        L->Register("print"_embed, EMBED_FUNC(PIL::StdLib_Print));
        BOOL result = RunScriptAndCheckResult(L, L"tests/scripts/state/minimal_setup.pil"_embed);
        delete L;
        return result;
    }
    ...
    static BOOL TestStateLifecycle()
    {
        ...
        for (INT32 i = 0; i < 3; i++)
        {
            PIL::State* L = CreateScriptState();
            L->Register("print"_embed, EMBED_FUNC(PIL::StdLib_Print));
            if (!RunScriptAndCheckResult(L, L"tests/scripts/state/lifecycle.pil"_embed))
            {
                delete L;
                return FALSE;
            }
